...
Accepted exceptions (stale-while-revalidate caches): a GET handler may kick off a bounded, fire-and-forget background refresh of a read-through cache when no fresh cache exists, provided the handler itself stays read-only and returns immediately with cached/fallback copy, the refresh is single-flight (concurrent GETs share one refresh), and a TTL bounds its frequency. The cache may hold generated content or an upstream read; what the rule protects is the response, which must never wait on the refresh and must never carry a write. Current instances:
...
- `GET /v1/plugins` and `GET /v1/plugins/search`: serve the plugin catalog from memory (or the bundled manifest on a cold daemon) and call `revalidatePluginCatalogInBackground()`, which fetches the platform catalog at most once per TTL per ref, backs off after a failure, and publishes `sync_changed(plugins:list)` when the refresh changes what the ref serves so clients refetch. Installs do not read this copy: they resolve their pinned source through `getAuthoritativePluginCatalog()`, which awaits the platform and fails hard.
