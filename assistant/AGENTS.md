...
`CES_LOCAL_SOCKET` is intentionally included despite the socket exposing credential RPCs: assistant subprocesses are expected to reach CES. Credential protection is rules-based access control inside CES, not socket-path secrecy (see root `AGENTS.md`). `CES_SERVICE_TOKEN` and `CES_CREDENTIAL_URL` are also forwarded: managed children still resolve connections through CES HTTP. Managed pods do not set `CES_LOCAL_SOCKET`, and that env is the lazy-RPC gate in `secure-keys.ts` (containerized discovery then dials `CES_BOOTSTRAP_SOCKET_DIR`). See the comment on those keys in `src/tools/terminal/safe-env.ts` before attempting to strip them.
