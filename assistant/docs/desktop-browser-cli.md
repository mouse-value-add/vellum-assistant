...
Chrome and its dock launcher share the managed profile and loopback debug port. Before launch, stale profile singleton files are removed only when their owner no longer uses the profile and their temporary socket is missing. Cookies and profile data remain intact across container replacement. For Chrome reopened from the dock, the profile singleton lock identifies a candidate PID, validated against the installed executable, profile and loopback debug arguments. Discovery checks `/proc` socket ownership against that browser PID, refuses redirects and validates the returned browser WebSocket endpoint. The connection stays within the container. No CDP endpoint or token is exposed to the renderer.
...
To verify recovery after packages disappear, run the Linux smoke script with
`--install /state/profile` in a disposable container. Replace that container with
a fresh one, preserving only the workspace and profile, then run
`--install /state/profile --expect-session`. The second run requires absent desktop
packages and an existing Chrome install marker, then verifies that one navigation
reinstalls the packages, opens the preserved profile, and sends its saved cookie.
