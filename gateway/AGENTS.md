...
- **Schema notes.** There is no `source_type` column: the wire DTO's `sourceType` is derived from `source_channel` (`phone` → voice, `vellum` → desktop, else channel), and the list route translates a `sourceType` filter into `source_channel` predicates. The requester-side conversation is `source_conversation_id`, and `source_turn_id` is the daemon turn whose tool call raised the request (null for rows that predate the column and for kinds no turn raises; written by the escalation helper and by a turn-emitted confirmation promotion, never by a route-generated one). Request ids are caller-supplied and load-bearing: the interaction-promotion paths (`tool_approval`, and `pending_question` for `ask_question`) reuse the pending-interaction id as PK, and every other create mints a UUID (voice `pending_question` rows carry their interaction id in `pending_question_id` instead). The insert is strict, so a caller that can create concurrently (the escalation path runs once per sibling tool call in a turn) must mint an id unique per call.
