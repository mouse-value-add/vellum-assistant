...
const BUNDLED_PACKAGES_TARGET = join(
  ROOT,
  "..",
  "assistant",
  "src",
...
function syncBundledPackages(): void {
  const expected = Buffer.from(
    `${JSON.stringify(
      { version: 1, packages: buildBundledPluginPackages(resolve(ROOT, "..")) },
      null,
      2,
    )}\n`,
  );
  if (!CHECK) {
    writeFileSync(BUNDLED_PACKAGES_TARGET, expected);
  }
}
...
if (CHECK) {
  ...
  console.log(`✓ Synced ${count} bundled copies and local plugin packages`);
}
