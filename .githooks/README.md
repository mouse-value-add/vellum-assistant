...
4. **ESLint**: Runs `eslint` on staged source files in `assistant/`, `cli/`, `gateway/`, `clients/docs/`, `clients/web/`, and `packages/design-library/`
...
- Runs prettier and eslint on staged files in assistant, cli, and gateway directories; runs eslint (without prettier) on clients/docs, clients/web, and packages/design-library
