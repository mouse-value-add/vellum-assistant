...
Wrap every `if` / `else` / `for` / `while` / `do…while` body in braces, even a single-statement one-liner. Braces make control flow easy to scan (the block boundary is explicit) and close a common footgun: a second line added under a braceless condition reads as if it sits inside the branch but runs unconditionally. The ESLint `curly` rule enforces this at `error` in `assistant/`, `clients/web/`, and `packages/design-library/`; it is fully auto-fixable with `eslint --fix`.
